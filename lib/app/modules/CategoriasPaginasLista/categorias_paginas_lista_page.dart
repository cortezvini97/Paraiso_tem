...
class CategoriasPaginasListaPage extends StatefulWidget {
  final String title;
  const CategoriasPaginasListaPage(
      {Key key, this.title = "CategoriasPaginasLista"})
      : super(key: key);

  @override
  _CategoriasPaginasListaPageState createState() =>
      _CategoriasPaginasListaPageState();
}

class _CategoriasPaginasListaPageState extends State<CategoriasPaginasListaPage>
{
  ...
  _tamanhoTela()
  {
    ...
    var tamanhos = {"ratio":1.2,"imagemaltura":"nulo", "fontesize": "nulo"};
    return tamanhos;
  }
  ...
  Future<List<PaginasModel>> _paginas() async
  {
    if(_filtro == "" || _filtro == null) {
      http.Response response = await http.get(_urlBase +
          "3UQucPNJXaePPsxCx1ctaVY8seWXP3HHcWH/Paginas/listPaginasPorCategoria/${widget.title}");
    ...
    {
      http.Response response = await http.get(_urlBase + "3UQucPNJXaePPsxCx1ctaVY8seWXP3HHcWH/Paginas/listPaginasPorFiltroCategoria/${widget.title}/${_filtro}");
      var dadosJson = json.decode(response.body);
